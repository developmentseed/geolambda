...
```
$ VERSION=1.1.0
$ docker build . --build-arg VERSION=${VERSION} --build-arg PYVERSION=3.6.5 -t <myimage>:latest
```

If not provided, `VERSION` (the version of GeoLambda to use) will default to `latest` and `PYVERSION` (Python version) will default to `3.6.5`.
...
This will copy site-packages in /geolambda/python/lambda directory needed for development. Also, it will create a `geolambda-python<pyversion>.zip` file in the current directory. 
...
$ aws lambda publish-layer-version \
	...
	--zip-file fileb://geolambda-python3.6.5.zip
```
